...
class ShellEmulator:
    ...
    def execute(self, command):
        if command.startswith("cd "):# 1
            ...
            cmd = command.split(" ")
            if len(cmd) == 3:
                self.cal_args(cmd[1],cmd[2])
            if len(cmd) == 5 and cmd[1] == '-d':
                self.cal_date(cmd[1], int(cmd[3]), int(cmd[4]))
            if len(cmd) == 5 and cmd[1] == '-y':
                print(cmd[1], int(cmd[2]), int(cmd[4]))
                self.cal_date(cmd[1], int(cmd[4]), int(cmd[2]))
...
'''
    ...
    def cal(self):
        ...
        monday_cal.prmonth(int(datetime.now().strftime("%Y")), int(datetime.now().strftime("%m")))
    
    def cal_args(self,arg,num):
        monday_cal = TextCalendar()
        if arg == "-A":
            for i in range (int(num)):
                monday_cal.prmonth(int(datetime.now().strftime("%Y")), (int(datetime.now().strftime("%m"))+i))
        if arg == "-B":
            for i in range (int(num)-1,-1,-1):
                monday_cal.prmonth(int(datetime.now().strftime("%Y")), (int(datetime.now().strftime("%m"))-i))
    
    def cal_date(self,arg,month,year):
        if arg == "-d" or arg == "-y":
            monday_cal = TextCalendar()
            monday_cal.prmonth(int(year), int(month))        
